...
import { DivMain, StyledTitle } from "./styled";
...
export default function Controller() {
  ...
  const handleSelected = (data) =>{
    if(data.type === "Client"){
      ...
      setNameSelected(data.name)
      if(data.checked){
        const contact = listContact.find(element => element.name === data.nome)
  ...
  const deleteContact = async () =>{
    if(nameSelected){
      const contacts = listContact.find(element => element.nome === nameSelected)
      const response = await custom_http.delete(`/contacts/${contacts.contact_id}`);
      if(response.status === 204){
        loadLists()
      }
    }
  ...
  const acessPageUpdate = (type) =>{
    ...
    if(type === "Client"){
      ...
      const contact = listContact.find(element => element.nome === nameSelected)
  ...
  return (
    <DivMain>
      <StyledTitle>
        <Typography variant="h6">Controler</Typography>
      </StyledTitle>
      <TabContext value={value} >
        ...
        <TabPanel value="1">
          <div>
            ...
            <Button disabled={!isSelected} onClick={() => acessPageRegisterContact()}>
              Cadastrar contato
            </Button>
            <Button disabled={!isSelected} onClick={() => deleteClient()}>
              Deletar
            </Button>
            <Button disabled={!isSelected} onClick={() => acessPageUpdate("Client")}>
              Alterar
            </Button>
          </div>
          <div>
            <BasicTable  type={"Client"} selected={handleSelected} data={listClient} />
          </div>
        </TabPanel>
        <TabPanel value="2">
          <div>
            ...
            <Button disabled={!isSelected} onClick={() => acessPageUpdate("Contact")}>
              Alterar
            </Button>
          </div>
          <div>
            <BasicTable type={"Contact"} selected={handleSelected} data={listContact}/>
          </div>
        </TabPanel>
      </TabContext>
    </DivMain>
