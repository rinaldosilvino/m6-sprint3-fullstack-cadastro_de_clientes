...
describe("Testes rota POST /clients", () => {
  it("Testando criação de usuário com um corpo correto", async () => {
    const response = await request(app).post("/clients").send(clientAdm);
...
describe("Testando rota GET /clients", () => {
  ...
  it("Testando listagem de usuários sem autorização", async () => {
    await request(app).post("/clients").send(clientNotAdm);
...
describe("Testando rota GET /clients/profile", () => {
...
describe("Testando rota PATCH /clients/<uuid>", () => {
  it("Testando atualização sem token", async () => {
    ...
    const response = await request(app)
      .patch(`/clients/${client.body.uuid}`)
      .send(updateNotAdm);
  ...
  it("Testando atualização do próprio usuário sem permissão de ADM", async () => {
    ...
    expect(response.body).toHaveProperty("email");
    expect(response.body).toHaveProperty("isAdm", client.body.isAdm);
    expect(response.body).not.toHaveProperty("password");
  });
  ...
  it("Testando atualização de qualquer usuário com permissão de ADM", async () => {
    ...
    const response = await request(app)
      .patch(`/clients/${notAdm.body.uuid}`)
...
describe("Testando rota DELETE /clients/<uuid>", () => {
  it("Testando deleção sem token", async () => {
    ...
    const response = await request(app).delete(`/clients/${client.body.uuid}`);
  ...
  it("Testando deleção de outro usuário com permissão de ADM", async () => {
    ...
    const response = await request(app)
      .delete(`/clients/${notAdm.body.uuid}`)
      .set("Authorization", `Bearer ${tokenAdm}`);
  ...
  it("Testando deleção do próprio usuário", async () => {
    ...
    const response = await request(app)
      .delete(`/clients/${client.body.uuid}`)
      .set("Authorization", `Bearer ${token}`);
